...
def _load_raw_findings_by_tool(scan_id: str) -> dict[str, list]:
    # Reference/demo source until raw scan artifacts are persisted (future
    # slice). Real impl: load this scan's stored adapter outputs from the
    # object store, keyed by source_tool.
    import pipeline
    return pipeline.load_fixture_findings()
